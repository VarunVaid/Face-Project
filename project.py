...
def check_haarcascadefile():
    ...
    if exists:
        ...
        mess._show(title='fechar file missing', message='some file is missing.')
        window.destroy()
        ...
        while (True):
            ...
            for (x, y, w, h) in faces:
                ...
                cv2.imwrite("TrainingImage\ " + name + "." + str(serial) + "." + Id + '.' + str(sampleNum) + ".jpg",
                            gray[y:y + h, x:x + w])
                # display the frame
                cv2.imshow('Taking Images To Close the camera press Q', img)
...
def TrackImages():
    ...
    while True:
        ...
        cv2.imshow('Taking Attendance To Close the camera press Q', im)
...
menu.add_command(label="Change Password!",command=change_pass)
...
#main window------------------------------------------------
message3 = tkinter.Label(window, text="Face Recognition Attendance System" ,fg="white",bg="#355454" ,width=60 ,height=1,font=('times', 29, ' bold '))
